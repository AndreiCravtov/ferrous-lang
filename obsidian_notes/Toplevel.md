    ...
    + Another thing I want to do is support robust "generic-eliding". In Rust there is already "lifetime-eliding" from
      ...
      which is an unnecessary conceptual fragmentation.
        ...
        * But I can't think of any intuitive syntax for this, since this language (as it stands) explicitly combines
          ...
          keyword into the signature itself. Maybe something like this??
            ```rust
            // in general, eliminate "bound" syntax and replace with consistent "type" synax
            // e.g. `x: T` means value `x` has type `T`
            //      `T: Type` means type `T` has kind `Type` and so on
            // 
            // Since Haskell is based on System F (System FC -> System F Omega -> System F)
            // I can model binder-syntax on System F's definition of types:
            //
            // Typ T ::= t
            //         | T1 -> T2
            //         | forall (t.T)
            //
            // So concluding from that, `for<..>` binders are types onto themselves, so with that 
            // insight `flip :: forall a b c. (a -> b -> c) -> b -> a -> c` becomes this
	        for<A: Type, B: Type, C: Type> fn flip(f: fn(A) -> fn(B) -> C)(b: B)(a: A) -> C {
		      f(a)(b)
	        }
	        
	        // The *kind* of generics should (usually) be inferred, so can write this instead
	        for<A, B, C> fn flip(f: fn(A) -> fn(B) -> C)(b: B)(a: A) -> C {
		      f(a)(b)
	        }
	        
	        // The generic parameters themselves should *also* be inferred, so write this instead
	        fn flip(f: fn(A) -> fn(B) -> C)(b: B)(a: A) -> C {
		      f(a)(b)
	        }
			
			// Finally, the last part of the the whole puzzle is how to specify bounds in a
			// consistent way?? Well, Haskell has syntax `forall t. c => T` which means that
			// the type `T` constrained by `c: Constraint` quantified by `t` so I have to
			// come up with an equivalent syntax using `for<..>` binders and `where` clause
		    // 
		    // Consider the function `foo :: forall a b. (Show a, a ~ b) => a -> (forall c. Eq c => a -> c) -> b`
		    for<A, B> fn foo(a: A)(f: for<C> fn(A) -> C where Eq<C>) -> B 
		    where Show<A>, A~B, // syntax sugar for constrant-tuple `(Show<A>, A~B)`
		    {..}
			
			// So from the example above, I've decided to somewhat flip Haskell's constraint-syntax
			// and instead do something like `forall t. (T <= c)` to keep with Rust's `where` convention
			//
			// The only other thing left to express is Quantified Constraints. In Haskell you can do e.g.
			// `nestEq :: (Eq a, forall b. (Eq b) => Eq (f b)) => f a -> f a -> Bool` 
			// but I decided flip the rule for quanitfied constraints, i.e. its `forall a. (T <= (c1, forall b. c2 => c3))`
			// I think that nicely tracks with Rust's GAT "bounds" syntax i.e. `type GAT<X: Trait>` is `type GAT<X where X: Trait>`
			//
			// so with that the function `nestEq` becomes  
			// `nestEq :: forall f a. f a -> f a -> Bool <= (Eq a, forall b. Eq b => Eq (f b))`
			// which becomes this in Rust-y syntax
			for<F: Type -> Type, A: Type> nest_eq(l: F[A])(r: F[A]) -> Bool
			where Eq[A], 
			      for<B where Eq[B]> Eq[F[B]]
			{..}
			
		    // Also there is somewhat of a syntactic tension between `<..>` and `[..]`;
		    // type-application via `T<A>` is ugly, I prefer `T[A]`;
		    // equally so, `for[T]` binders are ugly and I prefer `for<T>`;
		    // So the compromise is to associate `[..]` with type-application
		    // and associate `<..>` with type-quantification/type-binders
		    //
		    // so together thats the following three rules
		    // 1) semicolon is for typing; `x: T` means `x` is of type `T` (and same for type/kind)
		    // 2) angle-bracket is for binders; `for<T>` means quantification over types `T`
		    // 3) square-bracket is for type-application; `T[A]` means apply `A: Type` to type `T: Type -> Type`
      ...
      exclusivity checks) or maybe even `Rc<RefCell<T>>` syntax sugar??
        ...
        * Just to mention a few more random resources:
            ...
            - [Region-based Resource Management and Lexical Exception Handlers in Continuation-Passing Style](https://link.springer.com/chapter/10.1007/978-3-030-99336-8_18)
              ...
              lexical-regions, and relatedly transforming CPS to SSA
              with [Verified Transformation of Continuation-Passing Style into Static Single Assignment Form](https://dl.acm.org/doi/10.1007/978-3-031-35257-7_2).
...
- I **also** want to borrow the last-argument lambda syntax from Scala/Kotlin where if the last argument to a function is a lambda then it can be written "outside" that function's parameter list like this:
    ...
    ```
- I also want to have [delegation](https://kotlinlang.org/docs/delegation.html) syntax from Kotlin, which will make using [newtypes](https://doc.rust-lang.org/rust-by-example/generics/new_types.html) SO MUCH MORE SANE, maybe something like
    ```rust
    trait Foo {
	  fn foo(self) -> Self;
    }
	...
	// something like this could represent the the "delegate" syntax 
	struct NewBar(Bar);
	impl Foo for NewBar by self.0; // need to find better haha :)
	
	// under-the-hood it desugars into this
	struct NewBar(Bar);
	impl Foo for NewBar {
	  fn foo(self) -> Self {
	    NewBar(self.0.foo())
    ...
    ```
	+ This could also be made into a [macro](https://docs.rs/delegate/latest/delegate/) like `#[delegate(Foo by self.0)]` or something, and have more advanced usage...?
	- Kotlin also has "delegated properties" so look into that as well??
	- This feels like it could be represented as a programming pattern with HKTs tho... almost like Haskell's [monad transformers](https://en.wikibooks.org/wiki/Haskell/Monad_transformers)
		* There is essentially 3 steps to it => 1) lower the type 2) perform the operation 3) lift the result back
		* If I could define a mechanized/opt-in way of defining impls/instances on a type that delegates to a sub-type? Maybe this??
        ```rust
	    // Magical trait which marks a type as as delegating to another
	    trait INTERNAL_Delegate[T] {
		  type Rest;
			  
	      fn into_inner(self) -> (T, Self::Rest);
	      fn from_inner(inner: T, rest: Self::Rest) -> Self;
	    }
			
		// Magical functor-like trait which allows for "mapping" parametrically
		trait INTERNAL_Container
		where
			Self[_], // self is a higher-kinded type
		{
		  fn map[A,B](ta: Self[A], f: A -> B) -> Self[B]
	    }
		// Idk if I should auto-derrive it, or what, maybe that will break privacy abstractions (and thats not good)
		// but for now I will just implement it for `Option`
		impl INTERNAL_Container for Option[_] {
		  fn map[A,B](ta: Option[A], f: A -> B) -> Option[B] { .. }
		}
			 
		// All trait-definitions will implicitly generate blanket implementations like this
		trait Foo {
		  fn foo(self) -> Option[Self];
		}
		impl[K, T] Foo for K
		where 
			K: INTERNAL_Delegate[T],
			T: Foo,
		{
		  fn foo(self) -> Option[Self] { // we want `Option[K]`
		    let (inner, rest) = self.into_inner();
		    let rewrapper = |inner| K::rewrap(inner, rest);
		    inner.foo()
			    // somehow track down the `INTERNAL_Container` instance, and feed `rewrapper` to it
			    .map(rewrapper)
		  }
		}
			
		// And then via some kind of opt-in signaling (macros, keywords, something)
		// Struct-definitions will create implementations of `INTERNAL_Delegate` to their fields
		struct NewBar2(#[delegate] Bar, usize);
		
		// this would be auto-derrived or something...
		impl INTERNAL_Delegate<Bar> for NewBar {
		  type Rest = usize;
	      fn into_inner(self) -> (Bar, Self::Rest) {
	        let NewBar2(inner, rest) = self;
	        (inner, rest)
	      }
	      fn from_inner(inner: Bar, rest: Self::Rest) -> Self {
		    NewBar2(inner, rest)
	      }
		}
	    
        ```
	    * Obviously there are a lot of weird incomplete edgecases/possible breakages here, and also its starting to feel more and more like a [monad transformer](https://en.wikibooks.org/wiki/Haskell/Monad_transformers) of sorts in the sense that it has this "lift" property to it you are quite literally "lifting" and "lowering" these values (and whatever nested bullshit is inside them)
